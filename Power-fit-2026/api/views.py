...
def login_usuario(request):
    ...
    try:
        u = Registro.objects.get(correo=correo)

        password_ok = check_password(password, u.password)

        # Compatibilidad con usuarios antiguos que quedaron con contraseña en texto plano.
        # Si el login funciona, la contraseña se actualiza automáticamente a formato seguro.
        if not password_ok and u.password == password:
...
def coach_list(request):
    ...
    if request.method == 'POST':
        ...
        coach = NuevoCoach.objects.create(
            ...
            apellido=apellido,
            correo=correo,
            password=make_password(password),  # se guarda encriptada; login usa Django Auth
...
def coach_detail(request, coach_id):
    ...
    if request.method == 'PUT':
        ...
        serializer = nuevoCoachSerializer(coach, data=data)
        if serializer.is_valid():
            coach_actualizado = serializer.save()

            # Mantener sincronizado el usuario de Django Auth que se usa para iniciar sesión.
            user = User.objects.filter(username=coach.correo).first()
            if user:
                user.username = coach_actualizado.correo
                user.email = coach_actualizado.correo
                user.first_name = coach_actualizado.nombre
                user.last_name = coach_actualizado.apellido
                if nueva_password:
...
def reservar_clase(request):
    ...
    if str(usar_membresia).lower() in ["1", "true", "yes"]:
        ...
        if membresia.clases_disponibles is None:
            membresia.clases_disponibles = membresia.plan_clases
        ...
        # Descontar una clase desde clases_disponibles, no desde plan_clases.
        membresia.clases_disponibles -= 1
        if membresia.clases_disponibles <= 0:
            membresia.is_active = False
        membresia.save()

        return Response({"mensaje": "Clase reservada usando membresía", "reserva_id": reserva.id}, status=201)
...
def estadisticas_pagos(request):
    ...
    data = (
        Pago.objects
        .filter(fecha__gte=start_date, estado="Realizado")
        .values("fecha__month")
